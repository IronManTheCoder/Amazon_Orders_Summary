/**
 * BackgroundScanner Module
 * Handles multi-page scanning logic for the background script
 */
class BackgroundScanner {
  ...
  /**
   * Start tab navigation scan
   * @param {Object} scanData - Initial scan data
   * @param {number} originalTabId - Original tab ID
   */
  async startTabNavigationScan(scanData, originalTabId) {
    const { 
    ...
    try {
      ...
      for (let startIndex = 10; pageNum < (maxPages || this.maxPages); startIndex += 10) {
        ...
        await this.sendUpdateToTab(originalTabId, {
          totalAmount,
          totalOrderCount,
        ...
        try {
          ...
          if (results && results[0] && results[0].result) {
            ...
            if (pageResult.monthlyData) {
              allMonthlyData = this.mergeMonthlyData(allMonthlyData, pageResult.monthlyData);
            }
  ...
  /**
   * Send update message to tab
   * @param {number} tabId - Tab ID
   * @param {Object} data - Data to send
   * @param {string} type - Message type
   */
  async sendUpdateToTab(tabId, data, type) {
    try {
      await chrome.tabs.sendMessage(tabId, { type, data });
    } catch (error) {
      console.log('Could not send message to tab (may be closed)');
    }
  }

  /**
   * Merge monthly data from different pages
   * @param {Object} existing - Existing monthly data
   * @param {Object} newData - New monthly data to merge
   * @returns {Object} Merged monthly data
   */
  mergeMonthlyData(existing, newData) {
    const merged = { ...existing };
    
    for (const [month, data] of Object.entries(newData)) {
      if (!merged[month]) {
        merged[month] = { total: 0, count: 0 };
      }
      merged[month].total += data.total;
      merged[month].count += data.count;
    }
  ...
  /**
   * Get the function to inject into each page for scanning
   * This function will be injected into each tab and must be self-contained
   * @returns {Function} Page scan function
   */
  getPageScanFunction() {
    // This function will be injected into each tab
    // It needs to be self-contained with all necessary logic
    return function scanCurrentPageOrders() {
      let totalAmount = 0;
      let orderCount = 0;
